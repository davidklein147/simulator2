...
function userController() {
    function createUser(req, res) {
        ...
        newIntern.save(function (err, newDoc) {
            ...
            res.status(201).send({ _id: newToken._id, name: newToken.name,roleNum: newDoc.roleNum, token: newToken.token  });
            console.log(newDoc);
        });
    }
    ...
    function login(req, res) {
        ...
        function (err, doc) {
            ...
            if (!enCoding.compareCipher(req.body.password, doc.register.password)){
                ...
                res.status(200).send({_id: doc._id, name: doc.details.name, roleNum: doc.roleNum, token: newToken.token})
            })
    }
